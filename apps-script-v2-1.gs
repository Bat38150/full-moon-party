...
function setup() { const ss=SpreadsheetApp.getActiveSpreadsheet(); let sh=ss.getSheetByName(SHEET_NAME); if(!sh) sh=ss.insertSheet(SHEET_NAME); sh.getRange(1,1,1,HEADERS.length).setValues([HEADERS]); if(sh.getLastRow()<2) { const rows=INITIAL_TASKS.map(t=>[t.id,t.theme,t.task,t.responsable,t.status,t.echeance,t.commentaires,new Date()]); sh.getRange(2,1,rows.length,HEADERS.length).setValues(rows); } sh.setFrozenRows(1); sh.autoResizeColumns(1,HEADERS.length); return true; }
function doGet(e) { setup(); const callback=e.parameter&&e.parameter.callback; const payload={ok:true,tasks:getTasks_()}; return output_(payload, callback); }
function doPost(e) { setup(); let body={}; try { if(e.parameter&&e.parameter.payload) body=JSON.parse(e.parameter.payload); else if(e.postData&&e.postData.contents) body=JSON.parse(e.postData.contents); } catch(err) { body={}; } if(body.action==='upsert'&&body.task) upsertTask_(body.task); return output_({ok:true}, null); }
function getTasks_() { const sh=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAME); const values=sh.getDataRange().getValues(); if(values.length<=1) return []; return values.slice(1).filter(r=>r[0]).map(r=>({id:String(r[0]||''),theme:String(r[1]||''),task:String(r[2]||''),responsable:String(r[3]||''),status:String(r[4]||'À faire'),echeance:formatDate_(r[5]),commentaires:String(r[6]||'')})); }
function upsertTask_(task) { const sh=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAME); const last=sh.getLastRow(); const ids=last>1?sh.getRange(2,1,last-1,1).getValues().flat().map(String):[]; const row=[task.id,task.theme,task.task,task.responsable,task.status,task.echeance,task.commentaires,new Date()]; const i=ids.indexOf(String(task.id)); if(i>=0) sh.getRange(i+2,1,1,HEADERS.length).setValues([row]); else sh.appendRow(row); }
